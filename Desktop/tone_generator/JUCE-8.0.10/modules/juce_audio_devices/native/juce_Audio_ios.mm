...
struct iOSAudioIODevice::Pimpl final : public AsyncUpdater
{
    ...
    {
        // Bypass AudioQueue entirely - use AVAudioSession directly
        // This avoids the AudioQueueNew -50 error on iOS Simulator
        @autoreleasepool {
            AVAudioSession* s = [AVAudioSession sharedInstance];

            // Ensure session is active before querying (no-ops if already active)
            NSError* err = nil;
            if (![s isOtherAudioPlaying]) // optional
                [s setActive:YES error:&err];

            double sr = s.sampleRate;
            // Graceful fallback for Simulator / odd routes
            if (sr <= 0.0)
                sr = 48000.0;
